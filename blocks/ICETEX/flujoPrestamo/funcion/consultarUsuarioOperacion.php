...
if(!isset($_REQUEST['seleccionIdentificacion']))$_REQUEST['seleccionIdentificacion'] = 'cedula';
...
$datosArray['opcionConsulta'] =  $_REQUEST['opcionConsulta'];
...
switch($_REQUEST['opcionConsulta']){
	case 'cedula':
...
if($registrosD[$ix]!=null){
	...
	$_REQUEST['valorConsulta'] = $registrosD[$ix][0]['Codigo'];
}
...
if($valido==0){
	...
	$this->miMensaje->addMensaje("4","errorUsuarioNoEncuentra","error");
...
if($registrosD==null){
	...
	$this->miMensaje->addMensaje("4","errorConsultaDeuda","error");
	echo $this->miMensaje->getLastMensaje();
}